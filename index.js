...
function getSummary(character) {
  ...
  // const starredIn = character.films.length();

  // const bio = character.map(name => {
  ...
  // height: name.height,
  // mass: name.mass,
  // "film count": "Featured in" + starredIn + "films."
  // };
  // });
...
function getVehiclesCostInCreditsSumTotal(character) {
  ...
  const credSum = character.reduce(vehicles => {
    return vehicles.cost_in_credits;
  });
...
function getStarshipPassengerAndCrewSumTotal(character) {
  ...
  const allCraft = character.map(vehicles => {
    return vehicles.starships;
  });

  const allSum = character.reduce(allCraft => {
    return allCraft.cost_in_credits;
  });
...
function getNthFilm(character, filmNumber) {
  ...
  let favoriteM = "Forest Gump";

  const swTrilogy = character.filter(filmNumber => {
    if (filmNumber <= 3) {
      return `filmNumber`;
    } else
      return "There are only 3 Star Wars movies." + favoriteM + " excluded.";
  });
